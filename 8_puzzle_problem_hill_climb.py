...
def run():
    try:
        ...
        explored = 0
        first_node = first
        while (first_node.data != final_state):
            frontier = []
            heapify(frontier)
            heappush(frontier, first_node)
            ...
            min_heuristic_node = heappop(frontier)
            if(min_heuristic_node.heuristic<first_node.heuristic):
                visited.append(min_heuristic_node)
                first_node = min_heuristic_node
            else:
                print("\nTotal Nodes Explored : ", explored)
                print('\nUnable to find path, exiting...')
                sys.exit(0)
        visited.reverse()
        print_results(initial_state, backtrack(first_node, initial_state, visited),
                    explored)
    except SystemExit:
        exit(0)
    except:
