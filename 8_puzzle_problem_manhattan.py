...
class Node():
    ...
    def heurstic_calc(self):  #manhattan distance
        manhattan = 0
        for i in self.data:
            source = np.where(np.reshape(self.data, (3, 3)) == i)
            destination = np.where(np.reshape(final_state, (3, 3)) == i)
            dist = abs(source[0] - destination[0]) + abs(source[1] -
                                                         destination[1])
            manhattan += dist
